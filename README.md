...
默认正文用思源宋体（Noto Serif SC，SIL OFL 1.1）。系统自带的宋体是印刷字体，笔画细、横竖对比大，屏幕上小字号偏淡；思源宋体是为屏幕重新设计的宋体，笔画更稳。

字体文件随扩展一起分发（`fonts/`，常规 400 + 半粗 600，约 6 MB），和酒馆同源加载，不走 jsDelivr 等外部 CDN。它按 unicode-range 切成约 100 片，浏览器只下载页面上出现的字所在的片，一屏中文通常十几片。电脑里已经装了思源宋体时直接用本机的，不下载。
